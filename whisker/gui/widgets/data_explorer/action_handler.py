...
from whisker.gui.dialogs import (
    ...
    WarnIfExistsDialog,
    ImportLabelsDialog,
    ImportWhiskerBundleDialog,
    ExportWhiskerBundleDialog,
)
...
class ActionHandler(QObject):
    ...
    # --- Whisker bundles: export / import ---

    def show_export_dialog(self, preselect: Optional[str] = None):
        """Export datasets, with their media and labels, as a Whisker bundle (.zip)."""
        if not self._workspace:
            return
        ...
        if not self._ok_to_export():        # unsaved label edits are settled first: the export copies what is on disk
            return
        dialog = ExportWhiskerBundleDialog(self._workspace, self._active_project_name(), preselect, self.parent_widget)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        plan, dest = dialog.plan, dialog.destination
        if plan is None or dest is None:
            return
        overwrite = False
        if dest.exists():
        ...
        self._run_bundle_job(
            ExportBundleJob(self._workspace, plan, dest, overwrite),
            "Exporting",
    ...
    def _on_bundle_exported(self, result: dict):
        text, complete = self._describe_export(result)
        (QMessageBox.information if complete else QMessageBox.warning)(
            self.parent_widget, "Export Complete" if complete else "Export Finished With Warnings", text
        )

    @staticmethod
    def _describe_export(result: dict) -> Tuple[str, bool]:
        """``(message, complete)`` for a finished export. ``complete`` is False if some media weren't found or reading
        the bundle back found a problem."""
        names = result["datasets"]
        msg = (f"Exported {len(names)} dataset(s) to:\n{result['path']}\n\n"
               f"{result['num_files']} files, {wb.format_bytes(result['bytes'])}.")
        complete = True
        if result.get("num_missing"):
            complete = False
            where = ", ".join(f"'{n}' ({len(v)})" for n, v in result["missing"].items())
            msg += (f"\n\n{result['num_missing']} media file(s) couldn't be found on disk and are not in the bundle: {where}. "
                    "They are listed as missing in it.")
        problems = result.get("problems") or []
        if problems:
            complete = False
            msg += "\n\nThis bundle did NOT pass its check:\n" + "\n".join(f"• {p}" for p in problems)
        elif complete:
            msg += "\n\nChecked: the bundle was read back and every file passed its checksum."
        return msg, complete
    ...
    def show_import_dataset_dialog(self):
        ...
        if not self._workspace:
            ...
            return
        dialog = ImportWhiskerBundleDialog(self._workspace, self.parent_widget)
        if dialog.exec() != QDialog.DialogCode.Accepted or dialog.bundle_path is None:
            return
        self._run_bundle_job(
            ImportBundleJob(self._workspace, dialog.bundle_path, dialog.chosen, dialog.media_root),
            "Importing",
    ...
    def _refresh_after_import(self):
        ...
        MessageBus.get().publish("request/workspace/datasets/refresh")
        MessageBus.get().publish("request/workspace/labels/refresh")
        MessageBus.get().publish("request/workspace/predictions/refresh")
    ...
    def _describe_import(result: dict) -> str:
        """Plain-English summary of what an import did."""
        lines = []
        for key, heading in (("datasets", "Datasets"), ("labels", "Labels for"), ("runs", "Prediction results"),
                             ("projects", "Projects")):
            if result.get(key):
                lines.append(f"{heading}: " + ", ".join(f"'{n}'" for n in result[key]))
    ...
    def _handle_dataset_context_menu(self, item, pos, tree):
        try:
            ...
            if item_type == ItemTypeEnum.DATASET_BASE:
                ...
                # Export as a Whisker bundle (media + labels, optionally projects and prediction results).
                if dataset:
                    export_action = menu.addAction("Export Whisker Bundle...")
                    export_action.triggered.connect(
                        lambda: self.show_export_dialog(preselect=dataset_name)
                    )
