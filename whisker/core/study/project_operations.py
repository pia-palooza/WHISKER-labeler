...
class ProjectOperations(BaseOperationHelper):
    ...
    def scan_projects(self):
        ...
        logging.info(f"[ProjectOperations] Completed scanning projects. Found {len(self._projects)} projects.")
        for project in self._projects.values():
            logging.debug(
                f"  - {project.name} ({len(project.body_parts)} "
                f"Body Parts, {len(project.identities)} Identities)"
            )
    ...
    def remove(self, project_name: str):
        if project_name in self._projects:
            ...
            if project_path.exists():
                project_path.unlink()
            logging.info(f"Removed project {project_name}")
    
    def prune(self) -> dict[Path, str]:
        """
        Scans for corrupt or incomplete project entries.
        Returns:
            dict[Path, str]: Mapping of file path to reason for pruning.
        """
        findings = {}
        ...
        for project_file in os.listdir(self._base_dir):
            ...
            try:
                with open(project_path, "rb") as f:
                    Project.from_json(f.read().decode())
            except Exception as e:
                findings[project_path] = f"Corrupt project JSON: {e}"
