...
from whisker.core.study.dataset import Dataset, DatasetType, IMAGE_FILE_EXTENSIONS
...
class PoseLabelOperations(BaseWorkflowOperationsHelper):
    ...
    def scan_labels(self):
        ...
        if self._base_dir.exists():
            ...
            logging.info(
                f"[PoseLabelOperations] Completed scanning for pose labels. "
    ...
    def list_labeled_datasets(self) -> set[str]:
        """Names of every dataset that has pose ground-truth labels.

        Type-agnostic — includes VIDEO_COLLECTION, IMAGE_COLLECTION, and
        FRAME_SUBSET datasets alike.
        """
        return set(self._pose_labels.keys())
    ...
    def get_pose_labeled_image_keys_from_summary(self, dataset_name: str) -> set[str]:
        ...
        if metadata:
            if "frame_indices" in metadata:
                return set(metadata["frame_indices"])
            if "annotated_images" in metadata:
    ...
    def fill_labels_for_subsample_dataset(self, target_dataset_name: str, source_dataset_names: list[str]):
        ...
        for frame_key in target_dataset.files:
            if frame_key in filled_keys:
                raise ValueError(f"Duplicate frame key '{frame_key}' in target dataset '{target_dataset_name}' files.")
            ...
            for src_name, src_pose_ds in sources.items():
                if frame_key in src_pose_ds.frame_indices:
                    ...
                    logging.error(f"Frame '{frame_key}' not found in source dataset '{src_name}'.")
        ...
        logging.info(f"Filled {len(filled_keys)} out of {len(target_dataset.files)} frames for target dataset '{target_dataset_name}'.")
    ...
    def _create_dataset_for_import(self, dataset_name: str, pose_labels_path: Path, warn_if_exists, images_subdir: Optional[str] = None):
        ...
        dataset_dir = self._datasets.base_dir / dataset_name
        dataset_dir.mkdir(parents=True, exist_ok=True)

        relative_files: list[str] = []
        for src_file in sorted(files):
            rel_path = src_file.relative_to(pose_labels_path)
        ...
        new_dataset = Dataset(
            name=dataset_name,
            base_data_path=str(dataset_dir.resolve()),
            type=dataset_type,
            files=relative_files,
        )
        self._datasets.add_dataset(dataset_name, new_dataset)

        logging.debug(f"Copying {len(files)} image files to dataset directory: {dataset_dir}")

        for rel_path in relative_files:
            src_file = pose_labels_path / rel_path
            dst_file = dataset_dir / rel_path
            dst_file.parent.mkdir(parents=True, exist_ok=True)
            logging.debug(f"Copying: {src_file} -> {dst_file}")
            shutil.copy2(src_file, dst_file)
        logging.debug(f"Successfully copied {len(relative_files)} files")
    ...
    def get_pose_dataset(
        ...
        if self._pose_labels[dataset_name] is None:
            logging.debug(f"[PoseLabelOperations] Loading pose labels for dataset '{dataset_name}'...")
    ...
    def prune(self) -> dict[Path, str]:
        """
        Scans for corrupt or incomplete pose labels.
        Returns:
        ...
        for dataset_name in os.listdir(self._base_dir):
            ...
            label_dir = self._base_dir / dataset_name
            if not label_dir.is_dir(): continue

            labels_h5_path = label_dir / "labels.h5"
            metadata_path = label_dir / "metadata.json"

            # 1. Missing files
            if not labels_h5_path.exists() and not metadata_path.exists():
                findings[label_dir] = "Empty label directory (missing both .h5 and metadata.json)"
                continue
            
            if not labels_h5_path.exists():
                findings[label_dir] = "Missing labels.h5"
                continue

            if not metadata_path.exists():
                findings[label_dir] = "Missing metadata.json"
                continue

            # 2. Corrupt HDF5
            try:
                PoseDataset.from_file(labels_h5_path)
            except Exception as e:
                findings[label_dir] = f"Corrupt labels.h5: {e}"
                continue
            ...
            try:
                ...
                findings[label_dir] = f"Corrupt metadata.json: {e}"
